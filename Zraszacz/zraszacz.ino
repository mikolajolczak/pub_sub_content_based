...
#define UDP_SERVER_PORT         2140
#define UDP_CLIENT_PORT         2137
#define UDP_CLIENT_IP           192,168,56,103
#define switchPinSprinkler         ZSUT_PIN_D0
...
byte MAC[] = {0x8A, 0x6C, 0x5F, 0x51, 0x28, 0x1A}; //MAC adres karty sieciowej, to powinno byc unikatowe - proforma dla ebsim'a
...
void setup() {
  ...
  Serial.println();
  ZsutPinMode(switchPinSprinkler, OUTPUT);
  ZsutDigitalWrite(switchPinSprinkler, HIGH);
  hello = 0;
...
void loop() {
  ...
  if (msg.type == HUMIDITY) {
    ...
    if (msg.message < 5 && hum_on == 0) {
      hum_on = 1;
      ZsutDigitalWrite(switchPinSprinkler, HIGH);
      Serial.println(F("Wlaczono zraszacz"));
      memset(&msg, 0, sizeof(msg));
    }
    if (msg.message > 15 && hum_on == 1) {
      hum_on = 0;
      Serial.println(F("Wylaczono zraszacz"));
      ZsutDigitalWrite(switchPinSprinkler, LOW);
      memset(&msg, 0, sizeof(msg));
    }
  }
